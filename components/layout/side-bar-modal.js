    ...
    <SideDrawer showModal={showSideModal} setShowModal={setShowSideModal}>
        ...
        <ul className="flex flex-col justify-start items-start text-black text-base font-semibold font-inter mt-6 ml-5">    
          <li onClick={() => setViewModal(false)}>
             <Link href="/" className="flex gap-x-4 text-black font-bold mb-4 hover:text-red-950 items-center justify-center">
            ...
            {navmenu.map((nav) => (
              <li key={nav.name} onClick={() => setViewModal(false)}>
                <Link
                  ...
                  className={`flex gap-x-5 text-gray-900 mb-5 hover:text-red-950 ${pathName === nav.link &&
...
export function useSideBarModal() {
  ...
  return useMemo(
    () => ({ setShowSideModal: setShowSideModal, SideModal: SideModalCallBack }),
